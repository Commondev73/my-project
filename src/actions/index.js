...
export const FETCH_SEARCH_MAIL_REQUEST = "FETCH_SEARCH_MAIL_REQUEST";
export const FETCH_SEARCH_MAIL_SUCCESS = "FETCH_SEARCH_MAIL_SUCCESS";
export const FETCH_SEARCH_MAIL_ERROR = "FETCH_SEARCH_MAIL_ERROR";

export const fetchSearchMail = (data, page) => async (dispatch) => {
  await dispatch({ type: FETCH_SEARCH_MAIL_REQUEST });
  try {
    const formdata = new FormData();
    if (data !== "null") formdata.append("keyword", data);
    const response = await httpClient.post(
      `/api/mail/search?page=${page}`,
      formdata
    );
    return dispatch({
      type: FETCH_SEARCH_MAIL_SUCCESS,
      data: response.data,
    });
  } catch (err) {
    return dispatch({ type: FETCH_SEARCH_MAIL_ERROR, err });
  }
};

export const FETCH_SEARCH_ANNOUNCES_USER_REQUEST =
  "FETCH_SEARCH_ANNOUNCES_USER_REQUEST";
export const FETCH_SEARCH_ANNOUNCES_USER_SUCCESS =
  "FETCH_SEARCH_ANNOUNCES_USER_SUCCESS";
export const FETCH_SEARCH_ANNOUNCES_USER_ERROR =
  "FETCH_SEARCH_ANNOUNCES_USER_ERROR";

export const fetchSearchAnnouncesUser = (data, page) => async (dispatch) => {
  await dispatch({ type: FETCH_SEARCH_ANNOUNCES_USER_REQUEST });
  try {
    const formdata = new FormData();
    if (data.keyword !== "null") formdata.append("keyword", data.keyword);
    if (data.atype !== "null") formdata.append("atype", data.atype);
    if (data.ptype !== "null") formdata.append("ptype", data.ptype);
    if (data.bedroom !== "null") formdata.append("bedroom", data.bedroom);
    if (data.toilet !== "null") formdata.append("toilet", data.toilet);
    const response = await httpClient.post(
      `/api/user/announces/search?page=${page}`,
      formdata
    );
    return dispatch({
      type: FETCH_SEARCH_ANNOUNCES_USER_SUCCESS,
      data: response.data,
    });
  } catch (err) {
    return dispatch({ type: FETCH_SEARCH_ANNOUNCES_USER_ERROR, err });
  }
};
...
export const fetchSearch = (data, page) => async (dispatch) => {
  await dispatch({ type: FETCH_SEARCH_REQUEST });
  try {
    const formdata = new FormData();
    if (data.keyword !== "null") formdata.append("keyword", data.keyword);
    if (data.atype !== "null") formdata.append("atype", data.atype);
    ...
    if (data.bedroom !== "null") formdata.append("bedroom", data.bedroom);
    if (data.toilet !== "null") formdata.append("toilet", data.toilet);
    if (data.price !== "null" && data.toprice !== "null") {
      formdata.append("price", data.price);
      formdata.append("toprice", data.toprice);
    }
    const response = await axios.post(
      `/api/announces/search?page=${page}`,
      formdata
    );
    return dispatch({
    ...
    return dispatch({ type: FETCH_SEARCH_ERROR, err });
  }
};
...
export const fetchUserAnnounces = (page) => async (dispatch) => {
  ...
  try {
    const response = await httpClient.get(
      `/api/user/online/announces?page=${page}`
    );
    return dispatch({
...
export const FETCH_DRAFT_ANNOUNCES_USER_REQUEST =
  "FETCH_DRAFT_ANNOUNCES_USER_REQUEST";
export const FETCH_DRAFT_ANNOUNCES_USER_SUCCESS =
  "FETCH_DRAFT_ANNOUNCES_USER_SUCCESS";
export const FETCH_DRAFT_ANNOUNCES_USER_ERROR =
...
export const fetchUserAnnouncesDraft = (page) => async (dispatch) => {
  ...
  try {
    const response = await httpClient.get(
      `/api/user/draft/announces?page=${page}`
    );
    return dispatch({
...
export const FETCH_CORRECT_ANNOUNCES_USER_REQUEST =
  "FETCH_CORRECT_ANNOUNCES_USER_REQUEST";
export const FETCH_CORRECT_ANNOUNCES_USER_SUCCESS =
  "FETCH_CORRECT_ANNOUNCES_USER_SUCCESS";
export const FETCH_CORRECT_ANNOUNCES_USER_ERROR =
...
export const fetchUserAnnouncesCorrect = (page) => async (dispatch) => {
  ...
  try {
    const response = await httpClient.get(
      `/api/user/correct/announces?page=${page}`
    );
    return dispatch({
...
export const FETCH_BOOKMARKSID_REQUEST = "FETCH_BOOKMARKSID_REQUEST";
export const FETCH_BOOKMARKSID_SUCCESS = "FETCH_BOOKMARKSID_SUCCESS";
export const FETCH_BOOKMARKSID_ERROR = "FETCH_BOOKMARKSID_ERROR";

export const fetchBookMarksID = () => async (dispatch) => {
  await dispatch({ type: FETCH_BOOKMARKSID_REQUEST });
  try {
    const response = await httpClient.get(`/api/user/bookmark`);
    return dispatch({
      type: FETCH_BOOKMARKSID_SUCCESS,
      data: response.data,
    });
  } catch (err) {
    return dispatch({ type: FETCH_BOOKMARKSID_ERROR, err });
  }
};
...
export const updateAnnounces = (id, data) => async (dispatch) => {
  ...
  try {
    ...
    formdata.append("status", data.status);
    if (data.imageDelete) {
      for (let i = 0; i < data.imageDelete.length; i++) {
        formdata.append("delete_image[]", data.imageDelete[i]);
      }
    }
    for (let i = 0; i < data.image.file.length; i++) {
      formdata.append("image[]", data.image.file[i], data.image.file[i].name);
